...
def fix_number_of_points(points, target_n=512):
    """
    points : tensor (N, 3)
...
class PointCloudDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # ----------------------------------------------------
        # ### À MODIFIER ICI SI FORMAT DIFFÉRENT ###
        # ----------------------------------------------------
...
def test2(model, dataset, output_folder="predictions"):
    l_moy = []
    print("\n====== TEST ======\n")
    model.eval()
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            # Si tes vecteurs font 34 valeurs, on suppose que le dernier param est angle3
...
def load_and_predict(weights_path, cloud_to_test_dir, params_reference_dir):
...
if __name__ == "__main__":
    ...
    model = load_and_predict(
        weights_path="pointnet_arbre.pth", 
        
        cloud_to_test_dir="C:/Chemin/Vers/Nouveaux/Points/",
        params_reference_dir="D:/Chemin/Vers/Nouveaux/Json/"
    )
